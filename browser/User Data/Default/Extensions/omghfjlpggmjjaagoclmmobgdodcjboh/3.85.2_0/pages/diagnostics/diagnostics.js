(()=>{"use strict";var e,t={3940:(e,t,s)=>{var n=s(4168);const i=(e,t)=>{let s=performance.now(),n=self.pageYOffset;return new Promise((i=>{let o=()=>{let r=performance.now()-s;r>t?i():(self.scrollTo(0,n+(e-n)*r/t),requestAnimationFrame(o))};requestAnimationFrame(o)}))};var o=s(8897);let r=("undefined"!=typeof browser?browser:chrome).runtime.connect({name:"diagnostics"}),a=[];r.onMessage.addListener((e=>{a.forEach((t=>{t(e)}))}));const c=e=>{a.push(e)},d=async()=>{let e=await(0,o.Z)({type:"Diagnostics.getState"});if(!e)throw new Error("Connection with background failed");return e},l=async()=>{let e=await(0,o.Z)({type:"Diagnostics.getSteps"});if(!e)throw new Error("Connection with background failed");return e},p=e=>{a=a.filter((t=>t!==e))},h=e=>(0,o.Z)({type:"Diagnostics.start",extensions:e}),u=()=>(0,o.Z)({type:"Diagnostics.terminate"}),g=async e=>{if(!await(async()=>"undefined"!=typeof browser?browser.permissions.request({permissions:["management"]}):chrome.permissions.request({permissions:["management"]}))())throw new Error("Not enough permissions");if("undefined"!=typeof browser)return[];const t=(()=>{if("function"==typeof chrome.management.setEnabled)return chrome;const e=document.querySelector("#managementIframe");return e?e.contentWindow.chrome:void 0})();return t?(await Promise.all(e.map((e=>new Promise((s=>{t.management.setEnabled(e,!1,s)}))))),new Promise((e=>{t.management.getAll(e)}))):[]},x="\np, ul, ol, td, th{\n  padding: 0;\n  margin: 0;\n}\n\ntable {\n  border-collapse: collapse;\n}\n\nul, ol{\n  list-style: none;\n}\n";var b=s(5693),y=s(7584);const v=Object.fromEntries(Object.entries({checkConflictsWithOtherExtensions:"check_conflicts_with_other_extensions",checkBrowsecApi:"check_browsec_api",checkHttpInternetConnection:"check_http_internet_connection",checkHttpInternetConnectionWithBrowsec:"check_http_internet_connection_with_browsec",checkHttpsInternetConnection:"check_https_internet_connection",checkHttpsInternetConnectionWithBrowsec:"check_https_internet_connection_with_browsec",checkProxySettings:"check_proxy_settings",closePage:"close_page",fixIt:"fix_it",running:"running_dots",start:"start",tryAgain:"try_again",weHaveDetectedOtherExtensionsWhichBlockAccess:"we_have_detected_other_extensions_which_block_access",weHaveDetectedOtherExtensionsWhichCanConflict:"we_have_detected_other_extensions_which_can_conflict",youCanEnableTheseExtensionsLater:"you_can_enable_these_extensions_later"}).map((([e,t])=>[e,(0,b.Z)(t)]))),f=(e,t,s)=>{if(!t)return!1;let n=e.find((({name:e})=>"proxyApi"===e));return!n||"error"===n.state===("proxyApi"===s)},m=(e,t)=>{var s;switch(null===(s=e.find((({name:e})=>e===t)))||void 0===s?void 0:s.state){case"not started":default:return"";case"in process":return"progress";case"success":return"success";case"partial":return"partial";case"error":return"error"}},w=(e,t)=>{var s;let n=null===(s=e.find((({name:e})=>e===t)))||void 0===s?void 0:s.requests;if(!n)return"";let{success:i,total:o}=n;return i&&i!==o?i+"/"+o:""},k=(e,t)=>{let s=e.find((({name:e})=>e===t));if(!s)throw new Error(`Step ${t} not found!`);return s.state};function $({possibleSteps:e}){return y.dy`
  ...
  ${(()=>this.runningSteps?y.dy`
    <div class="MainStatus_Running">${v.running}</div>`:"")()}

  ${(()=>this.allStepsComplete?y.dy`
    <div class="MainStatus_Button">
      <input
        type="button"
        value="${v.tryAgain}"
  ...
  <div class="Steps">
  ${(()=>e.includes("proxyApi")?y.dy`
    <div class="E ${m(this.state,"proxyApi")}">
      <step-state
        .state="${k(this.state,"proxyApi")}"
        .completion="${w(this.state,"proxyApi")}"></step-state>
      <div class="Steps_Name">${v.checkProxySettings}</div>
    ${(()=>f(this.state,this.extensionsVisible,"proxyApi")?y.dy`
      <div class="Extensions"><div class="In">
        <div class="Extensions_Text">${v.weHaveDetectedOtherExtensionsWhichBlockAccess}</div>
        <c-extensions
          .extensions="${this.extensions}"
          @extensions-update="${this.extensionsUpdate}"></c-extensions>
        <div class="Extensions_Later">${v.youCanEnableTheseExtensionsLater}</div>
      </div></div>`:"")()}
    </div>`:"")()}

  ${(()=>e.includes("noProxyExtensions")?y.dy`
    <div class="E ${m(this.state,"noProxyExtensions")}">
      ...
      <div class="Steps_Name">${v.checkConflictsWithOtherExtensions}</div>
    ${(()=>f(this.state,this.extensionsVisible,"noProxyExtensions")?y.dy`
      <div class="Extensions"><div class="In">
        <div class="Extensions_Text">${v.weHaveDetectedOtherExtensionsWhichCanConflict}</div>
        <c-extensions
          .extensions="${this.extensions}"
          @extensions-update="${this.extensionsUpdate}"></c-extensions>
        ${"undefined"!=typeof browser?"":y.dy`
          <div class="Extensions_Later">${v.youCanEnableTheseExtensionsLater}</div>`}
      </div></div>`:"")()}
    </div>`:"")()}

  ${(()=>e.includes("httpConnection")?y.dy`
    <div class="E ${m(this.state,"httpConnection")}">
      ...
      <div class="Steps_Name">${v.checkHttpInternetConnection}</div>
    </div>`:"")()}

    ${(()=>e.includes("httpsConnection")?y.dy`
    <div class="E ${m(this.state,"httpsConnection")}">
      ...
      <div class="Steps_Name">${v.checkHttpsInternetConnection}</div>
    </div>`:"")()}

  ${(()=>e.includes("browsecApi")?y.dy`
    <div class="E ${m(this.state,"browsecApi")}">
      <step-state
        .state="${k(this.state,"browsecApi")}"
        .completion="${w(this.state,"browsecApi")}"></step-state>
      <div class="Steps_Name">${v.checkBrowsecApi}</div>
    </div>`:"")()}

  ${(()=>e.includes("httpProxyConnection")?y.dy`
    <div class="E ${m(this.state,"httpProxyConnection")}">
      ...
      <div class="Steps_Name">${v.checkHttpInternetConnectionWithBrowsec}</div>
    </div>`:"")()}

  ${(()=>e.includes("httpsProxyConnection")?y.dy`
    <div class="E ${m(this.state,"httpsProxyConnection")}">
      ...
      <div class="Steps_Name">${v.checkHttpsInternetConnectionWithBrowsec}</div>
    </div>`:"")()}
  </div>
  ...
  ${(()=>this.allStepsComplete?y.dy`
    ...
    <div class="FinalActionButton">
      ...
      />
    </div>`:this.blockedByAntivirus?y.dy`
    <div class="FinalActionButton">
      <input
        type="button"
        value="${v.tryAgain}"
    ...
    <c-logs></c-logs>`:"")()}`}const _=(0,b.Z)("disable");function S(){return y.dy`
  <style>
  ${x}
  :host{
    display: block;
  ...
  ${this.views.map(((e,t)=>y.dy`
    <tr>
      ...
      <td>
        ${(()=>e.icon?"undefined"!=typeof browser?y.dy`
        <div class="Icon"></div>`:y.dy`
        <img
          src="${e.icon}"
          width="38"
          height="38"
  ...
  ${(()=>this.fixable?y.dy`
    <div class="Fixable ${L.upperFirst(this.ownState)}">${"en"===e?"Health Check has found issues which can be fixed.":"Диагностика выявила проблемы, которыее можно исправить."}</div>`:this.blockedByAntivirus?y.dy`
    <div class="Fixable ${L.upperFirst(this.ownState)}">${"en"===e?y.dy`Browsec connection may be blocked by an <b>antivirus</b>.<br/>Please disabling the antivirus or contact our support team for advice.<br/>Then re-run Health check.`:y.dy`Возможно, работу Browsec блокирует <b>антивирус</b>.<br/>Попробуйте отключить антивирус или обратитесь в нашу службу поддержки за консультацией.<br/>Затем повторите Диагностику.`}</div>`:y.dy`
    <div class="Success">
      ${"en"===e?y.dy`
          No errors were found.<br/>
          Browsec should function normally.`:y.dy`
    ...
    <div class="Warning">
      ${"en"===e?y.dy`
          ...
          You can send Health Check logs to Browsec support to help diagnose these issues.`:y.dy`
